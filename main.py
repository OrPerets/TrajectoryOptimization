...
def infer_weights_auto(
    radar_xyz: np.ndarray, dt: float, eta: float, gamma: float
) -> Dict[str, float]:
...
def run_ablation_studies(
    ...
    sopts_qp = SolverOptions(
        ...
        alt_window_m=(
            None
            if cfg["solver"].get("alt_window_m") is None
            else float(cfg["solver"].get("alt_window_m"))
        ),
        xy_box_m=(
            None
            if cfg["solver"].get("xy_box_m") is None
            else float(cfg["solver"].get("xy_box_m"))
        ),
        alt_soft_penalty=float(cfg["solver"].get("alt_soft_penalty", 0.0)),
        xy_soft_penalty=float(cfg["solver"].get("xy_soft_penalty", 0.0)),
        ...
        for theta in theta_values:
            ...
            ocfg_theta = OuterConfig(
                ...
                cache_path=os.path.join(
                    artifacts_dir, f"outer_cache_theta_{int(theta)}.jsonl"
                ),
                plot_dir=os.path.join(out_dir, "figures"),
            )
...
def generate_all_figures(
    processed: Processed, results: Dict, ablation_data: Dict, out_dir: str
) -> None:
    ...
    if theta_results:
        ...
        if d_soc_by_theta:
            fig_theta_sensitivity(
                d_raw, d_qp, theta_values, d_soc_by_theta, figures_dir
            )
...
def generate_all_tables(
    processed: Processed, results: Dict, ablation_data: Dict, cfg: Dict, out_dir: str
) -> None:
    ...
    RMSE_qp = rmse(d_qp)
    TerminalMiss_qp = float(
        np.linalg.norm(res_qp.sol_best.p[-1] - processed.telem_xyz[-1])
    )
    # Raw baselines
    RMSE_raw = rmse(d_raw)
    TerminalMiss_raw = float(
        np.linalg.norm(processed.radar_xyz[-1] - processed.telem_xyz[-1])
    )

    has_soc = (
        res_soc is not None and res_soc.sol_best and res_soc.sol_best.p is not None
    )
    if has_soc:
        d_soc, _, _ = trajectory_errors(res_soc.sol_best.p, processed.telem_xyz)
        ...
        RMSE_soc = rmse(d_soc)
        TerminalMiss_soc = float(
            np.linalg.norm(res_soc.sol_best.p[-1] - processed.telem_xyz[-1])
        )
    else:
        CEP50_soc = None
        CEP90_soc = None
    ...
    metrics_row = {
        ...
        "TerminalMiss_soc": (
            TerminalMiss_soc if TerminalMiss_soc is not None else np.nan
        ),
    }
                ...
                theta_metrics.append(
                    {
                        ...
                        "TerminalMiss_soc": float(
                            np.linalg.norm(
                                r["solution"].p[-1] - processed.telem_xyz[-1]
                            )
                        ),
                    }
                )
    ...
    if not wilcoxon_results.empty:
        wilcoxon_results["P_value_formatted"] = wilcoxon_results["P_value"].apply(
            format_p_value
        )
        wilcoxon_results.to_csv(
            os.path.join(tables_dir, "wilcoxon_tests.csv"), index=False
        )
...
def run_once(args: argparse.Namespace) -> None:
    ...
    if cfg["weights"].get("auto_calibrate", True):
        W = infer_weights_auto(
            processed.radar_xyz,
            cfg["preprocessing"]["dt"],
            cfg["weights"]["eta"],
            cfg["weights"]["gamma"],
        )
    else:
    ...
    problem_data = ProblemData(
        ...
        weights=W,
        launch_fix=(
            processed.telem_xyz[0, :] if pre_align else processed.radar_xyz[0, :]
        ),
    )
    ...
    if res_qp.sol_best is None or res_qp.sol_best.p is None:
        ...
        if sol0.p is not None:
            ...
            tmp.k_best = k0
            tmp.J_best = (
                float(sol0.objective) if sol0.objective is not None else float("inf")
            )
            tmp.sol_best = sol0
            ...
            v_est = np.vstack(
                [
                    np.gradient(
                        (processed.radar_xyz + offset_vec)[:, i], problem_data.dt
                    )
                    for i in range(3)
                ]
            ).T
            tmp.sol_best = SimpleNamespace(
                p=(processed.radar_xyz + offset_vec), v=v_est, objective=None
            )
            res_qp = tmp
    ...
    df_sol = pd.DataFrame(
        {
            "time": processed.time,
            "x": res_qp.sol_best.p[:, 0],
            "y": res_qp.sol_best.p[:, 1],
            "z": res_qp.sol_best.p[:, 2],
        }
    )
    try:
    ...
    res_soc = results.get("soc_result")
    save_json(
        os.path.join(out_dir, "best_k.json"),
        {
    ...
    save_json(
        os.path.join(out_dir, "timing_summary.json"), ablation_data["timing_data"]
    )
    ...
    print(
        f"Figures generated: {len(os.listdir(os.path.join(out_dir, 'figures')))} files"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--enable_soc", type=lambda x: str(x).lower() == "true", default=None
    )
