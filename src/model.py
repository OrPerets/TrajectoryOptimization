...
class SolverOptions:
    ...
    alt_window_m: Optional[float] = 5.0
    xy_box_m: Optional[float] = 500.0
    alt_soft_penalty: float = 0.0
    xy_soft_penalty: float = 0.0
...
def build_problem(
    ...
    # Terminal altitude/XY constraints relative to anchor (telemetry) if provided, else observed terminal
    term_ref = (
        pd.terminal_anchor
        if pd.terminal_anchor is not None
        else positions_obs[T - 1, :]
    )
    if opts.alt_window_m is not None:
        alt_w = float(opts.alt_window_m)
        constraints += [
            p_s[T - 1, 2] >= (term_ref[2] - alt_w) / alpha_p,
            p_s[T - 1, 2] <= (term_ref[2] + alt_w) / alpha_p,
        ]
    if opts.xy_box_m is not None:
        box = float(opts.xy_box_m)
        constraints += [
    ...
    if opts.enable_soc:
        ...
        constraints += [
            cp.norm(v_s[T - 1, 0:2], 2) <= tan_theta * (-v_s[T - 1, 2]),
            v_s[T - 1, 2] <= 0,
        ]
    ...
    # Optional soft penalties toward terminal anchor
    if opts.alt_soft_penalty > 0:
        obj_terms.append(
            (opts.alt_soft_penalty * (alpha_p**2))
            * cp.square(p_s[T - 1, 2] - term_ref[2] / alpha_p)
        )
    if opts.xy_soft_penalty > 0:
        obj_terms.append(
            (opts.xy_soft_penalty * (alpha_p**2))
            * cp.sum_squares(p_s[T - 1, 0:2] - term_ref[0:2] / alpha_p)
        )
